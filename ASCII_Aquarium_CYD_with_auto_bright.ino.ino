// ASCII_Aquarium_CYD.ino | Version: v1.67 (With Auto-Brightness)
...
// --- AUTO BRIGHTNESS GLOBAL ---
float smoothedLight = 2000.0f; // Starting average for the light sensor
...
void setup() {
  ...
  // Setup backlight pin for Auto-Brightness
  pinMode(21, OUTPUT);
  // (Removed digitalWrite(21, HIGH); so the PWM can take over)
...
void loop() {
  // --- AUTO BRIGHTNESS START ---
  int rawLight = analogRead(34);

  // Smooth the reading so the screen doesn't flicker wildly from shadows
  smoothedLight = (smoothedLight * 0.95f) + (rawLight * 0.05f);

  // CUSTOM CALIBRATION: Dark (400) maps to Dim (10). Bright (0) maps to Max (255).
  int brightness = map((int)smoothedLight, 400, 0, 10, 255);
  
  // Constrain it so it never goes to pitch black or over 255
  brightness = constrain(brightness, 10, 255); 

  // Apply the brightness to the backlight pin
